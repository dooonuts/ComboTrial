...
void LCDDispStr(char inputstr[32])
{
    unsigned int i; //for while and for loops
    i=0;
    while(inputstr[i]!=NULL) //this is the work around to get strings 
    { //goes until that trigger char is reached
        LCDPutChar(inputstr[i]); //read the buffer inputstr array and output each char to the lcd
        i++; //iterate through the inputstr array
    }
}